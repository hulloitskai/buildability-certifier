...
function makePDF(name, email, award, identifier){
   ...
   let certificateImage = 'other_assets/certificate-template/template-blank.jpg';
...
function sendCertificateEmail(clientName, emailAddress, identifier){
   ...
   server.send(message, function(err, message){
      ...
      if (fs.existsSync('generated-content/' + identifier + '.pdf')) {
         fs.unlink(('generated-content/' + identifier + '.pdf'), function(error){
            if(error){
...
function backupExcelFile(){
   let files = fs.readdirSync('userdata-backups');
   let largestFileID = 0;
   files.forEach((file) => {
   ...
   fs.createReadStream(appStorage.get('excel-file-directory')).pipe(fs.createWriteStream('userdata-backups/' + newFileID + '.xlsx'));
   ...
   if (newFileID > maxBackAmount){
      ...
      for(i = 1; i <= itemsToSetBack; i++){
         fs.unlinkSync('userdata-backups/' + i + '.xlsx');
         console.log("Deleting " + i + ".xlsx");
      }

      files = fs.readdirSync('userdata-backups');

      for(var i = itemsToSetBack + 1; i <= (newFileID); i ++) {
         fs.renameSync('userdata-backups/' + i + '.xlsx', 'userdata-backups/' + (i - itemsToSetBack) + '.xlsx')
      }
